#!/usr/bin/env node
/**
 * NomercyBot Interactive Setup Helper
 *
...
const BACKEND_DIR  = path.join(ROOT, 'nomercybot-server');
const FRONTEND_DIR = path.join(ROOT, 'nomercybot-app');
const APPSETTINGS_DEV = path.join(BACKEND_DIR, 'src', 'NomercyBot.Api', 'appsettings.Development.json');
...
function writeEnvFile(filePath, vars) {
  const lines = [
    '# NomercyBot Environment Variables',
...
async function stepTwitch(config) {
  ...
  printBox([
    ...
    '',
    `  NomercyBot needs to be registered with Twitch so that Twitch`,
  ...
  if (skip) {
    console.log(dim('\n  Skipped. Set credentials manually in nomercybot-server/.env later.'));
    return { skipped: true };
  }
  ...
  printBox([
    `  ${bold('Name:')}      Anything you like — e.g. ${cyan('"My NomercyBot"')}`,
...
async function stepBotAccount(config) {
  ...
  printBox([
    ...
    '',
    `  NomercyBot can use a ${bold('separate Twitch account')} as the "bot".`,
  ...
  console.log(`  ${DIM}Example: MyChannelBot  or  StreamerBot_  or  NomercyBot_${RESET}`);
...
async function stepSpotify(config) {
  ...
  printBox([
    `  ${bold('App name:')}        Anything you like, e.g. ${cyan('"NomercyBot"')}`,
  ...
  const clientId = await getCredential({
    ...
    what: 'This tells NomercyBot which Spotify app to connect to.',
...
async function stepDiscord(config) {
  ...
  nl();
  step(4, `Give it any name (e.g. ${cyan('"NomercyBot"')}) and click ${bold('"Create"')}.`);
  nl();
...
async function writeBackendDotEnv(config) {
  ...
  if (exists) {
    nl();
    console.log(`  ${WARN} ${bold('nomercybot-server/.env')} already exists from a previous setup.`);
    ...
    if (c === 'm') {
      ...
      console.log(`  ${OK} nomercybot-server/.env — credentials merged in`);
      return;
    }
  }
  ...
  writeEnvFile(BACKEND_ENV, {
    POSTGRES_USER:                     'nomercybot',
    POSTGRES_PASSWORD:                 'nomercybot_dev',
    JWT_SECRET:                        generateSecret(32),
    JWT_ISSUER:                        'nomercybot',
    JWT_AUDIENCE:                      'nomercybot',
  ...
  console.log(`  ${OK} nomercybot-server/.env — ${exists ? 'overwritten' : 'created'}`);
}
...
function writeAppsettingsDev(config) {
  ...
  console.log(`  ${OK} nomercybot-server/src/NomercyBot.Api/appsettings.Development.json — updated`);
}
...
function writeFrontendEnv() {
  if (!fs.existsSync(FRONTEND_ENV)) {
    ...
    console.log(`  ${OK} nomercybot-app/.env.development — created`);
    return;
  }
  ...
  if (content.includes('bot-dev-api.nomercy.tv')) {
    ...
    console.log(`  ${OK} nomercybot-app/.env.development — API URL updated to localhost`);
  } else {
    console.log(dim('  nomercybot-app/.env.development — already configured'));
  }
}
...
async function stepStartServices(pkgMgr) {
  ...
  printBox([
    ...
    '',
    `  NomercyBot needs two background services running:`,
    '',
  ...
  if (!wantDocker) {
    ...
    console.log(dim('    cd nomercybot-server && docker compose up -d postgres redis adminer'));
  } else if (!fs.existsSync(path.join(BACKEND_DIR, 'docker-compose.yml'))) {
    console.log(`\n  ${FAIL} ${red('docker-compose.yml not found.')} Is the nomercybot-server folder missing?`);
  } else {
    nl();
    ...
    try {
      ...
      console.log(dim('  Try manually: cd nomercybot-server && docker compose up -d postgres redis adminer'));
    }
  ...
  if (startBackend) {
    const apiDir = path.join(BACKEND_DIR, 'src', 'NomercyBot.Api');
    launchInTerminal(`cd "${apiDir}" && dotnet run`, 'NomercyBot API');
    console.log(`\n  ${OK} API is starting in a new terminal window.`);
    console.log(`  ${DIM}Wait until you see "Now listening on: http://localhost:5080"${RESET}`);
  } else {
    console.log(dim('  Run manually: cd nomercybot-server/src/NomercyBot.Api && dotnet run'));
  }
  ...
  if (startFrontend) {
    ...
    launchInTerminal(`cd "${FRONTEND_DIR}" && ${cmd}`, 'NomercyBot Dashboard');
    ...
    const cmd = pkgMgr === 'yarn' ? 'yarn web' : 'npm run web';
    console.log(dim(`  Run manually: cd nomercybot-app && ${cmd}`));
  }
...
function printBanner() {
  ...
  console.log(`${MAGENTA}${BOLD}  ╚══════════════════════════════════════════════════════════╝${RESET}`);
  nl();
  console.log(dim('  This wizard will guide you through setting up NomercyBot step by step.'));
...
async function main() {
  ...
  nl();
  rule('═', GREEN);
  console.log(`${OK} ${bold(green('Setup complete! Enjoy NomercyBot.'))}`);
  rule('═', GREEN);
  nl();
